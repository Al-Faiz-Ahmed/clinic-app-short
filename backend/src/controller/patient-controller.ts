...
export const vCreatePateint = z.object({
  ...
  discount: z.number(),
  paid: z.number(),
}) satisfies z.ZodType<Omit<NewPatient, "id">>;
...
export const createPatient = async (
  ...
  const rqBody: Omit<NewPatient, "id"> = req.body;
...
interface GetPatientsQueryParams {
  page?: string;
  limit?: string;
  fromDate?: string;
  toDate?: string;
  from_date?: string; // Frontend sends this
  to_date?: string; // Frontend sends this
  doctorIds?: string | string[];
  doctor_ids?: string; // Frontend sends comma-separated string
  serviceIds?: string | string[];
  service_ids?: string; // Frontend sends comma-separated string
  patientName?: string;
  patient_name?: string; // Frontend sends this
  gender?: "male" | "female" | "other";
  age?: string;
  fixed_age?: string; // Frontend sends this
  minAge?: string;
  min_age?: string; // Frontend sends this
  maxAge?: string;
  max_age?: string; // Frontend sends this
}

// Response type for patient row (matching frontend expectations)
...
export const getPatients = async (
  ...
  try {
    const query = req.query as GetPatientsQueryParams;
    ...
    const patients: PatientRow[] = patientsResult.map((row) => ({
      id: row.id,
  ...
  try {
    ...
    const statsQuery = db
      .select({
        ...
        totalTodaySales: sql<number>`COALESCE(SUM(${patient.paid}), 0)`.as('totalTodaySales'),
      })
      .from(patient)
      .where(whereClause);
...
export const getCheckup = async (req: Request,
  res: Response,
  next: NextFunction,) => {
  try {
